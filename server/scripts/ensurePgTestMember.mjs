  ...
  try {
    ...
    if (found.rows.length === 0) {
      const ins = await client.query(
        ...
        RETURNING id`,
        [CUSTOMER_ID, PHONE, MEMBER_TYPE, DISPLAY_NAME, passwordHash]
      )
      ...
      await client.query(
        `UPDATE members SET
         ...
         WHERE id = $5`,
        [passwordHash, MEMBER_TYPE, DISPLAY_NAME, CUSTOMER_ID, memberId]
      )
    ...
    console.log('완료 — 로그인: 휴대폰', PHONE, '/ 비밀번호', PASSWORD)
  } finally {
    client.release()
    await pool.end()
  }
